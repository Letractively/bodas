<?php
	class ProveedorPublicacion extends Utilitarios{
		...
		public function obtenerPublicacionesXProveedor($id){
			$sql = "SELECT * FROM proveedores_publicaciones WHERE id_proveedor = ".$id." ORDER BY id_proveedor_publicacion DESC LIMIT 4";
			$qry = new Consulta($sql);
		...
		public function obtenerPublicacionesAntiguasXProveedor($id){
			$sql = "SELECT * FROM proveedores_publicaciones WHERE id_proveedor = ".$id." ORDER BY id_proveedor_publicacion DESC LIMIT 4,100";
			$qry = new Consulta($sql);
			while( $rw = $qry->VerRegistro() ){
				$rst[] = array(
					'id_proveedor_publicacion'		=> $rw['id_proveedor_publicacion'],
